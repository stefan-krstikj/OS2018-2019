package Lab03_Threads_Syncronization;
...
public class TribeDinner {
	 ...
	 * 
	 */
	static Semaphore kazanAvailable; // proverka dali moze da se zeme od kazanot (max 3)
	static Semaphore daliImaHrana; // proverka dali ima hrana (max 1)
	static boolean kazanImaHrana;

	public static void init(int numBarbers) {
		kazanAvailable = new Semaphore(3);
		daliImaHrana = new Semaphore(1);
	}
	...
	static class TribeMember extends TemplateThread {
		...
		public void execute() throws InterruptedException {
			...
			daliImaHrana.acquire();
			if (!state.isPotEmpty()) {
				daliImaHrana.release();
				kazanAvailable.acquire();
				state.fillPlate();
				kazanAvailable.release();
				state.eat();

			} else {
				synchronized (TribeMember.class) {
					state.cook();
					daliImaHrana.release();
				}
			}
